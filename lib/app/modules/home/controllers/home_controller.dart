...
class HomeController extends GetxController {
  ...
  Future<void> _loadUpcomingReservations() async {
    try {
      ...
      final reservations = await reservationRepository.getFutureReservationsByUser(
  ...
  Future<void> _loadQuickStats() async {
    try {
      ...
      if (currentUser.value != null) {
        ...
        final userReservations = await reservationRepository.getReservationsByUser(
          currentUser.value!.id
        );
        stats['userReservations'] = userReservations.length;
        stats['activeReservations'] = userReservations.where((r) => r.isActive).length;
        
        // Si es staff, mostrar estadísticas generales
        if (currentUser.value!.isStaff) {
          final toolStats = await toolRepository.getToolsStats();
          final reservationStats = await reservationRepository.getReservationsStats();
          
          stats['totalTools'] = toolStats['totalTools'] ?? 0;
          stats['availableTools'] = toolStats['availableTools'] ?? 0;
          stats['todayReservations'] = reservationStats['todayReservations'] ?? 0;
